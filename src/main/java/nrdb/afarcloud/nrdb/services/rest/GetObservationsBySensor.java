...
public class GetObservationsBySensor {
    ...
    public Response getLatestObservationsBySensor(    			
    		...
    		HashMap<String, String> hConditions = TelemetryExtractor.getTelemetryConditions(nLimit, sEntityNames, sDevices, sServices, sTypes,  sProviders, nAltitude, sOrder, lCentroide_long, lCentroide_lat, nRadius, csvFormat);
    ...
    @Path("/historic/")
    @Produces({ "application/json", "application/csv" })
    @Operation(summary = "Retrieves the telemetry in a time interval from sensors that meet specific constraint. If no filter conditions are specified (other than 'altitude'), it retrieves a maximum of 100 observations per measurement.", description = "", tags={ "Query sensor telemetry interval" })
    @ApiResponses(value = { 
    		@ApiResponse(responseCode = "200", description = "Successful Operation"),
    		@ApiResponse(responseCode = "205", description = "Invalid input"),    		
    		@ApiResponse(responseCode = "5XX", description = "Unexpected error")
    ...
    public Response getHistoricObservationsBySensor(    			
