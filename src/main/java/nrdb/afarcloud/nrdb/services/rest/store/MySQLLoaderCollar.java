...
public class MySQLLoaderCollar extends MySQLLoader {
	

	
	static {
		sTableName = Constants.MySQL_COLLAR_TABLE;
		COLUMNS = Constants.MySQL_LIST_COLLAR_COLUMNS;
	}


	public MySQLLoaderCollar(MySQLDataAccess oMySQL) {
		super(oMySQL);
	...
	protected int _deleteOldData(HashMap<String, Object> hValue) {
		...
		String sQuery = "DELETE FROM " + sTableName + " WHERE " +
				Constants.MySQL_COLLAR_ID + "= ? AND " + 
				Constants.MySQL_TIME + "<= ?"
				;
		List <Object> lValue= new ArrayList<Object>();

		lValue.add(hValue.get(Constants.MySQL_COLLAR_ID));
